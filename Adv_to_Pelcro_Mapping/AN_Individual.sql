create table dev.pelcro_records.an as (/*Base Advantage Query for AN*/
...
select
    ...
    case when Expire.PublicationCode like '%AN1%' then 'Premium'
     ...
     when Expire.PublicationCode like '%AN3%' then 'AN3(Discontinued)'
     when Expire.PublicationCode like '%AN4%' then 'All Access'
     when Expire.PublicationCode like '%AC%' then 'Premium Canada'
    ...
    case when Expire.PublicationCode like '%AN3%' then 3
      ...
      when expire.publicationcode like '%AC%' then 0
    end as pub_level
...
where owningoffice like '%AN%' or owningoffice like '%AC%'),
...
select 
	...
	case when max(pub_level)=4 then 'All Access'
		 when max(pub_level)=3 then 'AN3 Discontinued'
...
select * from linked_row_agg_info where ordernumber='C9901684'
