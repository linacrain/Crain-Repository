...
select
    ...
    BillTo.EmailAddress,
    BillTo.CompanyName,
    case when Expire.OwningOrganizaion like '%AC%' then 'AC'
    ...
    case when expire.billingCurrency='CAD' then 40
      else Expire.Rate end as rate,
...
where expire.OwningOrganizaion like '%AC%'),
...
SELECT distinct
	...
	case when i.planamount is not null and isusd is true then i.planamount
       when i.planamount is not null and i.planamount>40 and iscad then 40
	when i.planamount is null and isusd is true then price
	when i.planamount is null and iscad is true and price>40 then 40
...
where s.businessunitcode = 'ANG' and s.productname like '%Canada%'
...
from advantage_list
group by shiptocustomernumber,owningoffice,ordernumber,publicationcode
),
...
select distinct
	...
	case when i.amountdue is not null and isusd is true then i.amountdue 
	      when i.amountdue is not null and iscad is true and i.amountdue>40 then 40
	      when i.amountdue is null and isusd is true then price
	      when i.amountdue is null and iscad is true then 40
	      else price
	end AS rate,
	i.amountpaid,
	i.amountdue,
