...
join (SELECT 
	  ...
	  where (c.start_date=expirationissuedate or c.start_date>startissuedate) or (c.start_date>=startissuedate and c.start_date<=expirationissuedate) or (expirationissuedate=expire_date) or (start_date<=startissuedate&expire_date<=expirationissuedate) or c.chargereferences like '%null%' or c.chargereferences is not null),
...
select * from renewal_row_no_in_between
